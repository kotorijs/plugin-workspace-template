import { type SessionMsg, Tsu, plugins, KotoriPlugin, MessageScope } from 'kotori-bot'
...
export class TestingPlugin extends KotoriPlugin<Tsu.infer<typeof TestingPlugin.schema>> {
  ...
  public static groupDecrease(session: SessionMsg) {
    session.quick([
  ...
  public echo(data: { args: [string]; options: { num: number } }, session: SessionMsg) {
    ...
    return session.format('返回消息：~ {0}', [data.args[0]])
  }
